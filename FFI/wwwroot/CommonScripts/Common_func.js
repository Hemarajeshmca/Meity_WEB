...
function webSession_Exit() {
    ...
    if (serverExit == "true") {
        sessioncheck();
        // $("#logout").click();  // logout function call for when browser got session timeout
    }
}
function sessioncheck() {
    debugger;
    kendoAlert("Your session has expired please relaunch the application.");
    chatLogout();
    //logoutmobile(); // logout function call for when browser got session timeout
}
function chatLogout() {
...
function sec_PermissionNew(ScreenId) {
    $("#btn_new").attr("disabled", true);
    $("#btn_Edit").attr("disabled", true);
    $("#btn_View").attr("disabled", true);
    $("#divEdit #deactivate").prop("disabled", true);
...
//hindi mastercode
function grid_comboScreen_hindi(mstcode) {
    var data = {};
    var list = [];
    data.screen_Id = mstcode;
    //var parent_code = ajaxcall('/Home/screenId_mastercodelist');
    ...
    if (mst_Dt.toString() != "null" && mst_Dt != "[]")
        list = getGridComboListhindi(JSON.parse(mst_Dt));//changed as getComboList
    return list;
}


//grid level combo
function getGridComboListhindi(mst_Dt) {
    var arr = [];
    $.each(mst_Dt, function (key, value) {
        var list = {};
        if (value != null) {
...
function exportfunction(e, chkall, chkfilter, chktemp, twokendobox, gd) {
    ...
    if (chkall == true) {
        ...
        if (e.sender.dataSource._view.length == 0) {
            ...
            sdata.griddata = JSON.stringify(data);
        }
    }
    else {
...
function page_load_list_permission(permission) {
    if (permission.deny == true) {
        ...
        if (permission.new == true && permission.auth == true) {
            $('#btn_new').attr("disabled", true);
            $('#btn_Edit').attr("disabled", false);
            $('#btn_View').attr("disabled", false);
        }
        if (permission.new == true && permission.view == true && permission.auth == false && permission.modify == false) {
            $('#btn_new').attr("disabled", false);
            $('#btn_Edit').attr("disabled", true);
            $('#btn_View').attr("disabled", false);
        }
        if (permission.new == true && permission.view == true && permission.auth == false && permission.modify == true) {
            $('#btn_new').attr("disabled", false);
            $('#btn_Edit').attr("disabled", false);
            $('#btn_View').attr("disabled", false);
        }
...
function master_get_hindi() {
    var exists = (typeof get_hindi_lang === 'function');
    if (exists == true) {
        get_hindi_lang();
    }
    else {
...
function getLanguageListMode(screen_id, grid_id, def, loc) {
    ...
    try {
        ...
        var values = ajax_set_context("/DynamicGrid/gridListCreation", JSON.stringify(data));
