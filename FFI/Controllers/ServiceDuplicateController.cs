...
{
    public class ServiceDuplicateController : Controller
    {
        // GET: ServiceDuplicate
        //Muralidharan developed on 12-05-2020
        private IConfiguration _configuration;
        string urlstring = "";
        public ServiceDuplicateController(IConfiguration configuration)
        {
            _configuration = configuration;
        }
        public ActionResult ServiceDuplicateList()
        {
            return View();
        }
        public ActionResult ServiceDuplicateForm()
        {
            return View();
        }
        public ActionResult ServiceDuplicateEdit()
        {
            return View();
        }
        public class List
        ...
        public ActionResult SingleServiceList([FromBody]SingleContext objContext)
        {
            ...
            using (var client = new HttpClient()) 
            {
                string Urlcon = "FISDublicateReq/";
                client.BaseAddress = new Uri(urlstring + Urlcon);
               // client.BaseAddress = new Uri("http://169.38.77.190:101/api/FISDublicateReq/");
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpContent content = new StringContent(JsonConvert.SerializeObject(objContext), UTF8Encoding.UTF8, "application/json");
                //var response = client.GetAsync("").Result;
                var response = client.PostAsync("service_req_duplicate", content).Result;
                Stream data = response.Content.ReadAsStreamAsync().Result;
                StreamReader reader = new StreamReader(data);
                post_data = reader.ReadToEnd();
        ...
        [HttpPost]
        public ActionResult SingleServiceDuplicateDetails([FromBody]ReqContext objContext)
        {
        ...
        [HttpPost]
        public ActionResult SaveReqDuplicate([FromBody]SaveContext objContext)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var response = client.PostAsync("newServiceRequestDuplicate", content).Result;
                Stream data = response.Content.ReadAsStreamAsync().Result;
                StreamReader reader = new StreamReader(data);
                post_data = reader.ReadToEnd();
            }
