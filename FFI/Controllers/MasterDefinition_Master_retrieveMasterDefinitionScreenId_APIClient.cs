...
{
    #region API (retrieveMasterDefinitionScreenId) INVOCATION 
    /// <summary>
     ...
     */
    public List<string> invokeAPI_Master_retrieveMasterDefinitionScreenId(dynamic contextrequest, dynamic Headerrequest, dynamic Headerresponse, ref dynamic Detailresponse)
    {
        ...
        try
        {
            ...
            dynamic param_screen_code = null;
            ...
            param_screen_code = Headerrequest.screen_code.ToString(); // Unicode string(50)
            
            string requestDataFormat = null;
            string responseDataFormat = "application/json";
            
            string targetURL = WebAPIProxy.getWebURL() + "/api/MasterDefinition/mastercode_screenid?org=" + param_orgnid + "&locn=" + param_locnid + "&user=" + param_userid + "&lang=" + param_localeid + "&screen_code=" + param_screen_code + "";
            ...
            {
                try {
                    processAPIResponse_Master_retrieveMasterDefinitionScreenId(response, Headerresponse, ref Detailresponse);
                }
                catch (Exception exception) {
                    Master_retrieveMasterDefinitionScreenIdhandleException("Error encountered while processing API response. " + exception.Message);
                }
            }
            else
            {
                ...
                if (errorNumber != "")
                    errorNumber += ": ";
                Master_retrieveMasterDefinitionScreenIdhandleException(errorNumber + errorDescription);
            }
        }
        catch (Exception exception)
        {
    ...
    protected void Master_retrieveMasterDefinitionScreenIdhandleException(string expMessage)
    {
        throw new Exception(expMessage);
    }
    #endregion
    
    #region PROCESSING API (retrieveMasterDefinitionScreenId) RESPONSE 
    //Code for processing response data from the API 'retrieveMasterDefinitionScreenId'
    private void processAPIResponse_Master_retrieveMasterDefinitionScreenId(dynamic responseJSON, dynamic Headerresponse, ref dynamic Detailresponse)
    {
        var outcontext = responseJSON.context;
        
        var outHeader = outcontext.Header;
        Headerresponse.screen_description = outHeader.screen_description.ToString();  //screen_description
        ...
        if(outDetail != null && outDetail.Count > 0)
        {
            ...
            if(Detailresponse.GetType().IsArray)
            {
                for(int DetailChildi = 0; DetailChildi < outDetail.Count; DetailChildi++)
                {
                    ...
                    Detail_arrayinst.parent_description = outDetail[DetailChildi].parent_description.ToString();    // parent_description
