...
namespace FFI.Controllers
{
    public class ServiceAddressController : Controller
    {
        ...
        public ActionResult ServiceAddressList()
        {
            return View();
        }
        public ActionResult ServiceAddressForm()
        {
            return View();
        }
        public ActionResult ServiceAddressEdit()
        {
            return View();
        }
        ...
        public ActionResult SingleServiceList([FromBody]SingleContext objContext)
        {
            ...
            using (var client = new HttpClient())
            {
                //string Urlcon = "/service_req_address?org=" + objContext.orgnId + "&locn=" + objContext.locnId + "&user=" + objContext.userId + "&lang=" + objContext.localeId + "&servicereq_rowid=" + objContext.Header.IOU_servicereq_rowid + "&fpomember_code=" + objContext.Header.IOU_fpomember_code + "&servicereq_no=" + objContext.Header.IOU_servicereq_no + "";
                //client.BaseAddress = new Uri(urlstring + Urlcon);
                //client.BaseAddress = new Uri("http://169.38.77.190:101/api/FISAddresschange/");
                string Urlcon = "FISAddresschange/";
                client.BaseAddress = new Uri(urlstring + Urlcon);
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpContent content = new StringContent(JsonConvert.SerializeObject(objContext), UTF8Encoding.UTF8, "application/json");
                //var response = client.GetAsync("").Result;
                var response = client.PostAsync("service_req_address", content).Result;
                Stream data = response.Content.ReadAsStreamAsync().Result;
                StreamReader reader = new StreamReader(data);
                post_data = reader.ReadToEnd();
        ...
        [HttpPost]
        public ActionResult SingleServiceAddressDetails([FromBody]ReqContext objContext)
        {
        ...
        [HttpPost]
        public ActionResult SaveReqAddress([FromBody]SaveContext objContext)
        {
            SingleApplication objList = new SingleApplication();
            ...
            using (var client = new HttpClient()) 
            {
                //string Urlcon = "/newServiceRequestAddress";
                //client.BaseAddress = new Uri(urlstring + Urlcon);
                //client.BaseAddress = new Uri("http://169.38.77.190:101/api/FISAddresschange/"); 
                string Urlcon = "FISAddresschange/";
                client.BaseAddress = new Uri(urlstring + Urlcon);
                ...
                var response = client.PostAsync("newServiceRequestAddress", content).Result;
                Stream data = response.Content.ReadAsStreamAsync().Result;
                StreamReader reader = new StreamReader(data);
                post_data = reader.ReadToEnd();
                objList = (SingleApplication)JsonConvert.DeserializeObject(post_data, typeof(SingleApplication));
