...
namespace FFI.Controllers
{
    public class ServiceBankController : Controller
    {
        ...
        string urlstring = "";
        public ServiceBankController(IConfiguration configuration)
        {
            _configuration = configuration;
        }
        public ActionResult ServiceBankList()
        {
            return View();
        }
        public ActionResult ServiceBankForm()
        {
            return View();
        }
        public ActionResult ServiceBankEdit()
        {
            return View();
        }
        public class List
        ...
        public ActionResult SingleServiceList([FromBody]SingleContext objContext)
        {
            ...
            using (var client = new HttpClient()) 
            {
                //string Urlcon = "/service_req_bank?org=" + objContext.orgnId + "&locn=" + objContext.locnId + "&user=" + objContext.userId + "&lang=" + objContext.localeId + "&servicereq_rowid=" + objContext.Header.IOU_servicereq_rowid + "&fpomember_code=" + objContext.Header.IOU_fpomember_code + "&servicereq_no=" + objContext.Header.IOU_servicereq_no + "";
                //client.BaseAddress = new Uri(urlstring + Urlcon);
                //client.BaseAddress = new Uri("http://169.38.77.190:101/api/FISBankchange/");
                string Urlcon = "FISBankchange/";
                client.BaseAddress = new Uri(urlstring + Urlcon);
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpContent content = new StringContent(JsonConvert.SerializeObject(objContext), UTF8Encoding.UTF8, "application/json");
                //var response = client.GetAsync("").Result;
                var response = client.PostAsync("service_req_bank", content).Result;
                Stream data = response.Content.ReadAsStreamAsync().Result;
                StreamReader reader = new StreamReader(data);
                post_data = reader.ReadToEnd();
        ...
        [HttpPost]
        public ActionResult SaveReqBank([FromBody]SaveContext objContext)
        {
            SingleApplication objList = new SingleApplication();
            ...
            using (var client = new HttpClient())
            {
                //string Urlcon = "/newServiceRequestBank";
                //client.BaseAddress = new Uri(urlstring + Urlcon);
                //client.BaseAddress = new Uri("http://169.38.77.190:101/api/FISBankchange/");
                string Urlcon = "FISBankchange/";
                client.BaseAddress = new Uri(urlstring + Urlcon);
                ...
                var response = client.PostAsync("newServiceRequestBank", content).Result;
                Stream data = response.Content.ReadAsStreamAsync().Result;
                StreamReader reader = new StreamReader(data);
                post_data = reader.ReadToEnd();
                objList = (SingleApplication)JsonConvert.DeserializeObject(post_data, typeof(SingleApplication));
