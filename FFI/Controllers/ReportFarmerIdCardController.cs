...
{
    public class ReportFarmerIdCardController : Controller
    {
        ...
        public ReportFarmerIdCardController(IConfiguration configuration, IHostingEnvironment hostingEnvironment)
        {
        ...
        // GET: ReportFarmerIdCard
        public ActionResult ReportFarmerIdCard()
        {
            return View();
        }
        ...
        public class FarmerInput
        {
            ...
            public string userId { get; set; }
            public string localeId { get; set; }
            public string In_fliterby { get; set; }
            public string In_from_date { get; set; }
            public string In_to_date { get; set; }
        }

        [HttpPost]
        //Get Download Pdf 19-09-2020
        public string downloadpdf(string orgnId, string locnId, string userId, string localeId, string In_fliterby, string In_from_date, string In_to_date)
        //public string downloadpdf(FarmerInput Objcontext)
        {

            dynamic PDF_detail = new JObject();
            PDF_detail.msg = "";
            string clientpath = "";
            PDF_detail.Clentpath = "";
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("orgnId", orgnId);
                cmd.Parameters.AddWithValue("locnId", locnId);
                cmd.Parameters.AddWithValue("userId", userId);
                cmd.Parameters.AddWithValue("localeId", localeId);
                cmd.Parameters.AddWithValue("In_fliterby", In_fliterby);
                cmd.Parameters.AddWithValue("In_from_date", In_from_date);
                cmd.Parameters.AddWithValue("In_to_date", In_to_date);
                MySqlDataAdapter da = new MySqlDataAdapter(cmd);
                da.Fill(table_data);
                logger.Debug("Farmer Id Card Screen DataTable Count -" + table_data.Tables[0].Rows.Count);
                con.Close();

                if (table_data.Tables[0].Rows.Count > 0)
                {
                    ...
                    clientpath = Path.Combine("/DownloadXLFiles/" + sptver + file_ext);
                    PDF_detail.Clentpath = clientpath;
                    ...
                    for (int i = 0; i < table_data.Tables[0].Rows.Count; i++)
                    {
                        ...
                        {
                            int pholength = Convert.ToInt32(table_data.Tables[0].Rows[i]["lenghtpho"].ToString());
                            //if (table_data.Tables[0].Rows[i]["lenghtpho"].ToString() != "0")
                            if (pholength > 100)
                            {
                                logger.Debug(table_data.Tables[0].Rows[i]["ID"].ToString());

                                base64photo = table_data.Tables[0].Rows[i]["Photo"].ToString();
                                imageBytesphoto = Convert.FromBase64String(base64photo);
                                iTextSharp.text.Image imagephoto = iTextSharp.text.Image.GetInstance(imageBytesphoto);
                                imagephoto.ScaleAbsolute(50f, 50f);
                                imageBytesphoto = null;
                                base64photo = "";
                                ...
                                Obj_FarmerQrContext.orgnId = orgnId;
                                Obj_FarmerQrContext.localeId = localeId;
                                Obj_FarmerQrContext.locnId = locnId;
                                Obj_FarmerQrContext.userId = userId;
                                String base64 = new_farmer_qr(Obj_FarmerQrContext);
                                // var response = base64.Substring(1, base64.Length - 2).Replace(@"\/", "/");
                                string64 = base64.Replace(@"""", string.Empty);
                                imageBytes = Convert.FromBase64String(string64);
                ...
                {
                    PDF_detail.msg = "No record";
                }
            }
            catch (Exception ex)
            {
                // throw ex;
                logger.Error("User Name -" + userId + "Method Name -" + methodName + ex);
            }

            //excel_msg.path = Clientpath;
            // return JsonConvert.SerializeObject(Clientpath); 
            return JsonConvert.SerializeObject(PDF_detail);
        }
